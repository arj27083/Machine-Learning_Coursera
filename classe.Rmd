...
percent(length(grep(TRUE, acc))/nrow(dat_test))
```



##**Conclusion**
The machine learning model has been built for the given datasets and cross-validation is done by splitting the training dataset in the ratio of 75:25. Our model shows `r percent(length(grep(TRUE, acc))/nrow(dat_test))` accuracy. This model can be applied on the testing dataset.
